...
>
> ALB Ingress 설정 시 흔한 함정인 'redirect loop'를 미리 방지하기 위해 ArgoCD server에 `--insecure` 플래그를 박았습니다. ArgoCD default가 HTTPS-only listen + HTTP→HTTPS auto-redirect라 ALB(HTTP)와 만나면 무한 redirect가 생기는데, 이걸 ArgoCD를 HTTP로 listen하게 바꿔 회피했습니다(현재 시연 단계라 ALB도 HTTP, ACM/HTTPS termination 추가는 Phase 5+). 또 별도 ALB(`group.name=argocd`)로 앱용 ALB와 분리해 운영 책임 영역을 명확히 했습니다.
>
